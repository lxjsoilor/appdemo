        ...
        <div class="bottom">
            <div class="item-1">
                <div class="shopping-car-box">
                    ...
                    >
                        <div class="ballbox" v-for="(isShow,index) in ballArray" :key="index" v-show="isShow.isShow">
                            <transition 
                            @before-enter="beforeEnter2"
                            @enter="enter2"
                            @after-enter="afterEnte2"
                            >
                                <div class="ballmove" v-show="isShow.isShow"></div>
                            </transition>
                        </div>
                    </transition-group>
                </div>
...
<script>
    export default {
        ...
        computed: {
            ballArray() {
                return this.$store.state.ballArray;
            }
        },
        methods: {

            beforeEnter(el) {
                el.style.transform = `translateY(${this.$store.state.ballX})`;
                var inner = document.getElementsByClassName('ballmove')[this.index - 1];
            },
            enter(el, done) {
                el.offsetHeight;
                var inner = document.getElementsByClassName('ballmove')[this.index - 1];
            ...
            afterEnter(el) {
                setTimeout(() => {
                    ...
                    this.ballArray[this.tempIndex].isShow = false;
                    this.tempIndex++;
                    var carbox = document.getElementsByClassName('shopping-car-box')[0];
                    var car = document.getElementsByClassName('car')[0];
                    var carClone = car.cloneNode(true);
                    carbox.removeChild(car);
                    carbox.appendChild(carClone);
            ...
            beforeEnter2(el) {
                el.style.transform = `translateX(${this.$store.state.ballY})`;
            },
            enter2(el) {
                el.offsetHeight;
...
<style>
    ...
    .ballbox {
        transition: all .5s cubic-bezier(.23, -0.4, .69, .73);
        width: 16px;
        height: 16px;
        left: 50%;
        margin-left: -8px;
    ...
    .ballmove {
        ...
        border-radius: 50%;
        transition: all 0.5s;
        /* transition-timing-function: bezier(.47, -0.44, .77, .88); */
    }
    ...
    .car {
        ...
        margin: 0 auto;
        margin-top: 6px;
        animation: goodscar .5s cubic-bezier(.22, -1.28, .58, 2.39) reverse;
    }
